...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        extracted_bbox = np.array(self.roi_data['boxes'][0][index])
        extracted_bbox_score = np.array(self.roi_data['boxScores'][0][index]).flatten()
        index_sorted = np.argsort(extracted_bbox_score)[-self.top_n:]
        extracted_bbox = extracted_bbox[index_sorted]
        extracted_bbox = extracted_bbox[:, [1,0,3,2]]
        proposals = np.zeros((extracted_bbox.shape[0], 4))

        proposals[:,0] = extracted_bbox[:,0]/width
        proposals[:,1] = extracted_bbox[:,1]/height
        proposals[:,2] = extracted_bbox[:,2]/width
        proposals[:,3] = extracted_bbox[:,3]/height
